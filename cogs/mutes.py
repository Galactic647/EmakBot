...
class Mutes(commands.Cog):
    ...
    async def mute(self, ctx, target: Union[discord.Member, str], reason: str, duration: Optional[str] = None) -> None:
        ...
        mute_list[msg_id] = mod.Mute(self.bot, ctx.author, target, duration, reason, msg_id)
        embed = func.embedder('Mute', msg_id, ctx.author.mention, target, target.avatar_url,
                              reason, duration, 'Server')
        await target.add_roles(mute_role)
        await func.process_message(self, ctx, embed, target, USER_PREFIX['mute'].format('Server', duration, reason))

        asyncio.ensure_future(mute_list[msg_id].count_down())
    ...
    async def unmute(self, ctx, target: Union[discord.Member, str], reason: str) -> None:
        ...
        await func.process_message(self, ctx, msg, mute_obj.user,
                                   USER_PREFIX['unmute'].format(mute_obj.type_, reason))
    ...
    async def lsmute(self, ctx, target: Union[discord.Member, str], reason: str,
        ...
        mute_list[msg_id] = mod.LSMute(self.bot, ctx.author, target, duration, reason, msg_id,
