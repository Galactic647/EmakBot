from globals import DEFAUT_DURATION, USER_PREFIX, ADMIN_PREFIX, ERROR_MESSAGE, mute_list
...
class Mutes(commands.Cog):
        ...
        try:
            for msg_id, mute_obj in mute_list.items():
                if user == mute_obj.user and 'Server' == mute_obj.type_:
                    ...
                    embed = func.embedder('Mute', msg_id, ctx.author.mention, user, user.avatar_url,
                    ...
                    logger.info(f'User {ctx.author} mute a user, info\n'
                                f'{json.dumps(mute_list[msg_id].info(), indent=4)}')
    ...
    async def lsmute(self, ctx, user: discord.Member, reason: str, duration: Optional[str] = None) -> None:
        ...
        try:
            ...
            embed = func.embedder('Mute', msg_id, ctx.author.mention, user, user.avatar_url, reason, duration,
                                  'Livestream')
